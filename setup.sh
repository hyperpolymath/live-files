...
# setup.sh — Universal setup script for live-files
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/live-files/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
main() {
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/live-files.git"
        info "Then: cd live-files && ./setup.sh"
        exit 1
    fi
