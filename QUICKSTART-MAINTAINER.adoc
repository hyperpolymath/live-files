...
// Replace live-files, live-files, {{DEPS}} with actuals
= live-files — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining live-files for
...
| `$PREFIX/share/live-files/`
| Data files, assets

| `$PREFIX/share/doc/live-files/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/live-files/config.toml`

Fallback: `$HOME/.config/live-files/config.toml`
...
Or via OPSM: `opsm update live-files`
...
[source,bash]
----
just install --prefix=/opt/live-files-instance1 --config=/etc/live-files/instance1.toml
just install --prefix=/opt/live-files-instance2 --config=/etc/live-files/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/live-files/issues
